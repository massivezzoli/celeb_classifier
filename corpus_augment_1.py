...
print('popping the last layers!')
new_input =  model.input
new_bot_layer = model.layers[-3].output
new_out_d = Dense(n_entities, name='fc_2')(new_bot_layer)
new_out_softmax = Activation('softmax', name='softmax')(new_out_d)
...
try:
    ...
    bottle_tensor_func = K.function([model2.layers[0].input, K.learning_phase()],
    ...
    score, accuracy = model2.evaluate([imgs_test, bots_test], lab_test_ohe, batch_size=50, verbose=0)
    print('Test score:', score)
    print('Test accuracy:', accuracy)
    epc_count= int(K.get_value(epoch_count))+1
    file_name = 'model_exp_'+str(epc_count)+'.h5'
